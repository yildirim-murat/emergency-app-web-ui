            ...
            <table className="table table-hover text-center user-select-none">
                ...
                {data && data.length > 0 ? (
                    data.map((incident, index) => (
                        <tr key={incident.id} className={"table-danger"} style={{cursor: 'pointer'}}>
                            ...
                            <td className={"text-truncate"} style={{maxWidth: "50px"}}>{incident.incidentId}</td>
                            <td className={"text-truncate"} style={{maxWidth: "50px"}}>Staff {index + 1}</td>
                            <td className={"text-truncate"} style={{maxWidth: "50px"}}>Type {index + 1}</td>
                            <td className={"text-truncate"} style={{maxWidth: "50px"}}>{incident.description}</td>
                            <td className={"text-truncate"} style={{maxWidth: "50px"}} title={incident.calledNumber}>{incident.calledNumber}</td>
                            <td className={"text-truncate"} style={{maxWidth: "50px"}}>{incident.createdAt}</td>
                            <td className={"text-truncate"} style={{maxWidth: "150px"}}>{incident.address.province} {incident.address.district} {incident.address.district}</td>
                            <td className={"text-truncate"} style={{maxWidth: "50px"}}>{incident.address.description}</td>
                            <td className={"text-truncate"} style={{maxWidth: "50px"}}>{incident.isPriority ? "Evet" : "Hayır"}</td>
                            <td className={"text-truncate"} style={{maxWidth: "50px"}}>Staff {index + 1}</td>
                        </tr>
                    ))
                ) : (
                    <tr>
                        <td colSpan="12">Veri bulunamadı</td>
                    </tr>
                )}
